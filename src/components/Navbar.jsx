...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 border-b-3 ${
        scrolled
          ...
          <div className="hidden md:flex items-center gap-2">
            {navLinks.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `px-4 py-2 font-bold uppercase text-sm border-2 border-transparent hover:border-primary hover:bg-highlight-blue hover:shadow-neo-sm transition-all rounded-lg ${
                    isActive
            ...
            <div className="bg-white border-3 border-primary shadow-neo rounded-xl p-4 space-y-2 mt-4">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block px-4 py-3 font-bold uppercase border-2 transition-all rounded-lg ${
                      isActive
                        ? 'bg-highlight-purple border-primary shadow-neo-sm'
