...
export default function VisionMission() {
  return (
    <section className="w-screen h-screen flex items-center bg-bg border-t-3 border-b-3 border-primary relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {pillars.map(({ icon: Icon, title, desc, color }, idx) => (
            ...
            <div className="text-center group h-full">
              <div className="bg-white border-3 border-primary shadow-neo rounded-neo p-6 h-full flex flex-col items-center hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all duration-200">
                <div
                  className={`w-20 h-20 ${color} border-3 border-primary rounded-full flex items-center justify-center mb-5 shadow-neo-sm group-hover:shadow-none transition-all`}
                >
                  <Icon size={32} className="text-primary" />
                </div>
                <h3 className="font-heading font-black text-2xl text-primary uppercase mb-3">{title}</h3>
