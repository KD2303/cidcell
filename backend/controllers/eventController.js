...
const registerForEvent = async (req, res) => {
    try {
        ...
        // Atomically increment and check capacity in one operation
        const updatedEvent = await Event.findOneAndUpdate(
            { _id: req.params.id, registeredCount: { $lt: event.maxAttendees } },
            { $inc: { registeredCount: 1 } },
            { new: true }
        );

        if (!updatedEvent) {
            // Event is full - remove the registration we just created
